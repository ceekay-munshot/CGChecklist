...
export async function judgeEvidence(
  ...
  const prompt =
    ...
    `- Scoring — if the question methodology above states its OWN scoring rule (e.g. "Score 0.5 if …, 0 if …", or explicit 0.5 / 0.25 / 0 bands), apply THAT rule EXACTLY and let it decide the score; it overrides the general calibration below. Otherwise calibrate like a discerning buy-side analyst, NOT a lenient one. Do not reflexively default to 0.5.\n` +
