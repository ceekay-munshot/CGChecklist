...
// the "What is expected" column of the client's Beas checklist sheet. A further
// FIVE questions use in-house prompts we drafted and validated against the
// client's manual report (see ADOPTED_INHOUSE_PROMPTS); the remaining twenty
// fall back to a rigorous default built from the same house style (verdict-
// first, exact figures, grounded, one Excel cell). Each bespoke
...
// ---------------------------------------------------------------------------
// ADOPTED in-house prompts — 5 of the checklist questions the client has NOT
// yet supplied a prompt for, where our in-house drafting measurably beat the
// generic default in an A/B against the client's manual Capillary report (most
// notably catching the SessionM cyber-fraud red flag). Written in the client's
// house style (verdict-first, exact figures, an explicit 0 / 0.25 / 0.5 band,
// one Excel cell) and now ALWAYS used, alongside the client's 26 bespoke
// prompts. The other 20 not-yet-client questions keep the generic default
// until the client supplies their own. Kept separate from DETAILED_PROMPTS
// (the client's own) to preserve provenance.
// ---------------------------------------------------------------------------
const ADOPTED_INHOUSE_PROMPTS: Record<string, string> = {
...
export function buildQuestionPrompt(
  ...
  // The client's 26 bespoke prompts win, then our 5 adopted in-house prompts
  // (validated against the client's manual). Every other question uses the
  // generic default below.
  const detailed =
    DETAILED_PROMPTS[questionId] ?? ADOPTED_INHOUSE_PROMPTS[questionId];
  const core = detailed
...
export const hasDetailedPrompt = (questionId: string): boolean =>
  questionId in DETAILED_PROMPTS || questionId in ADOPTED_INHOUSE_PROMPTS;
